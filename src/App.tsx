...
function App() {
  ...
  const { clientesVisibles, movimientosVisibles, chequesVisibles } =
    useMemo(() => {
      if (!user) {
        return {
          clientesVisibles: [],
      ...
      if (esAdminUser) {
        return {
          clientesVisibles: clientes,
          movimientosVisibles: movimientos,
          chequesVisibles: cheques,
        };
      }
      return {
        clientesVisibles: clientes.filter((c) => c.vendedor_id === user.id),
        movimientosVisibles: movimientos.filter(
          (m) => m.vendedor_id === user.id
        ),
  ...
  return (
    <div className="min-h-screen bg-secondary text-gray-800">
      ...
      <div className="flex">
        <aside
          className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:block w-64 bg-white shadow-sm border-r border-gray-200 min-h-screen absolute md:relative z-20 md:z-0`}
        >
            ...
            {esAdmin && (
              <div className="mb-4">
                ...
                <NavItem
                  ...
                  label="Vendedores"
                  to="/vendedores"
                  active={isActive('/vendedores')}
                />
                <NavItem
                  icon={<Calculator size={20} />}
                  label="Comisiones"
                  to="/comisiones"
                  active={isActive('/comisiones')}
                />
                <NavItem
                  icon={<DollarSign size={20} />}
                  label="Liquidaciones"
                  to="/liquidaciones"
                  active={isActive('/liquidaciones')}
                />
                <NavItem
          ...
          {isLoading && clientes.length === 0 ? (
            ...
            <Routes>
                ...
                element={
                  selectedClienteId ? (
                    <EstadoCuentaView
                      ...
                      currentUser={user}
                      onMovimientoChange={() => loadAll(user)}
                    />
                  ) : (
              ...
              {esAdmin && (
                <Route
                  path="/cheques"
                  element={<ChequesView currentUser={user} />}
                />
              )}
              {esAdmin && (
                <Route
                  path="/vendedores"
                  element={<VendedoresView currentUser={user} />}
                />
              )}
              {esAdmin && (
                <Route
                  path="/comisiones"
                  element={<ComisionesView currentUser={user} />}
                />
              )}
              {esAdmin && (
                <Route
                  path="/liquidaciones"
                  element={<LiquidacionesView currentUser={user} />}
                />
              )}
              {esAdmin && (
                <Route
                  path="/inventario"
                  element={<InventarioView currentUser={user} />}
                />
              )}
              {esAdmin && (
