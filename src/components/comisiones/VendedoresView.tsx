...
import { Plus, Edit, Trash2, UserCheck, Calculator, Phone, Mail, Calendar } from 'lucide-react'
    ...
    try {
      ...
      const vendedoresEnriquecidos = await Promise.all(
        data.map(async (vendedor) => {
          const { count } = await supabase.from('clientes').select('id', { count: 'exact' }).eq('vendedor_id', vendedor.id)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setVendedorEditando(null); setShowFormulario(true); }} className="bg-primary text-white px-4 py-2 rounded-md hover:bg-primary-dark flex items-center">
              ...
              {vendedores.map((vendedor) => (
                <tr key={vendedor.id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500 capitalize">{vendedor.rol}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium">{vendedor.comision_porcentaje}%</div>
                    <div className="text-sm text-gray-500">{formatearComisionBase(vendedor.comision_base)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm">{vendedor.clientes_asignados || 0}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${vendedor.activo ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                      {vendedor.activo ? 'Activo' : 'Inactivo'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                    <button onClick={() => { setVendedorEditando(vendedor); setShowFormulario(true); }} className="text-primary hover:text-primary-dark"><Edit size={16} /></button>
                    {vendedor.id !== currentUser.id && (
                      <button onClick={() => handleEliminarVendedor(vendedor)} disabled={eliminando === vendedor.id} className="text-red-600 hover:text-red-800">
                        {eliminando === vendedor.id ? '...' : <Trash2 size={16} />}
                      </button>
                    )}
                  </td>
                </tr>
      ...
      <FormularioVendedor isOpen={showFormulario} onClose={() => setShowFormulario(false)} onSuccess={handleVendedorGuardado} currentUser={currentUser} vendedorEditar={vendedorEditando} />
    </div>
  )
}
