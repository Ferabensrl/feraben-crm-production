...
export const FormularioMovimiento: React.FC<FormularioMovimientoProps> = ({
  ...
  const obtenerSaldoCliente = async (clienteId: number) => {
    try {
      ...
      const saldo =
        movimientos?.reduce((sum, mov) => sum + mov.importe, 0) || 0;
      setSaldoActual(saldo);
      console.log(
        `💰 Saldo del cliente ${clienteId}: ${formatearMoneda(saldo)}`
      );
    } catch (error) {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.tipo_movimiento)
      newErrors.tipo_movimiento = 'Tipo de movimiento requerido';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const movimientoData = {
        ...
        importe: ['Pago', 'Nota de Crédito', 'Devolución'].includes(
      ...
      alert(
        `✅ Movimiento ${movimientoEditar ? 'actualizado' : 'creado'} correctamente`
      );
  ...
  const saldoProyectado =
    saldoActual !== null ? saldoActual + formData.importe : null;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleInputChange('tipo_movimiento', e.target.value)
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange('importe', parseFloat(e.target.value) || 0)
                }
            ...
            <div>
              ...
              <input
                ...
                value={
                  clienteSeleccionado
                    ? `ID: ${clienteSeleccionado.vendedor_id}`
                    : currentUser.nombre
                }
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span
                    className={`font-medium ml-2 ${saldoProyectado && saldoProyectado > 0 ? 'text-red-600' : 'text-green-600'}`}
                  >
