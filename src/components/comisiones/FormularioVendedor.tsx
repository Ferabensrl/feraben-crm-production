...
export const FormularioVendedor: React.FC<FormularioVendedorProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        `✅ Vendedor ${esEdicion ? 'actualizado' : 'creado'} exitosamente.`
      );
      onSuccess();
      onClose();
    } catch (err: any) {
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h3 className="text-xl font-semibold text-gray-900">
            {vendedorEditar ? 'Editar Vendedor' : 'Nuevo Vendedor'}
          </h3>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary ${errors.nombre ? 'border-red-500' : 'border-gray-300'}`}
                disabled={loading}
              />
              {errors.nombre && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    max="100"
                    step="0.01"
                    className={`w-full border rounded-md px-3 py-2 pr-8 focus:outline-none focus:ring-2 focus:ring-primary ${errors.comision_porcentaje ? 'border-red-500' : 'border-gray-300'}`}
                    disabled={loading}
                  />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleInputChange('comision_base', e.target.value)
                  }
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary"
                  disabled={loading}
                >
                  <option value="pagos">Solo sobre pagos recibidos</option>
                  <option value="ventas">Solo sobre ventas realizadas</option>
                  <option value="ambos">Sobre ventas y pagos</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary ${errors.email ? 'border-red-500' : 'border-gray-300'}`}
                disabled={loading}
              />
              {errors.email && (
                <p className="text-red-500 text-xs mt-1 flex items-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary ${errors.fecha_ingreso ? 'border-red-500' : 'border-gray-300'}`}
                disabled={loading}
              />
            ...
            >
              {loading ? (
                <>
                  ...
                  {vendedorEditar ? 'Actualizando...' : 'Creando...'}
                </>
              ) : (
                <>
                  <UserCheck size={16} className="mr-2" />
                  {vendedorEditar ? 'Actualizar Vendedor' : 'Crear Vendedor'}
                </>
              )}
            </button>
          </div>
