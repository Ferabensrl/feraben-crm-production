...
export const calcularSaldoCliente = (movimientos: Movimiento[]): number => {
  return movimientos.reduce(
    (saldo, movimiento) => saldo + movimiento.importe,
    0
  );
};
...
export const getMovimientosCompleto = async (): Promise<Movimiento[]> => {
  try {
    ...
    while (hayMasDatos && intentos < MAX_INTENTOS) {
      ...
      logger.log(
        `✅ Lote ${intentos + 1} cargado: ${data.length} movimientos (total acumulado: ${todoLosMovimientos.length})`
      );
    ...
    if (intentos >= MAX_INTENTOS) {
      console.warn(
        '⚠️ Se alcanzó el máximo de intentos. Puede haber más datos.'
      );
    }

    // Ordenar por fecha descendente para mostrar los más recientes primero
    todoLosMovimientos.sort(
      (a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()
    );

    logger.log(
      `✅ TOTAL de movimientos cargados: ${todoLosMovimientos.length}`
    );
    logger.log(
      `📊 Primer movimiento: ${todoLosMovimientos[0]?.fecha} - ${todoLosMovimientos[0]?.documento}`
    );
    logger.log(
      `📊 Último movimiento: ${todoLosMovimientos[todoLosMovimientos.length - 1]?.fecha} - ${todoLosMovimientos[todoLosMovimientos.length - 1]?.documento}`
    );
...
export const getMovimientosPorCliente = async (
  clienteId: number
): Promise<Movimiento[]> => {
  try {
    ...
    logger.log(
      `✅ Movimientos del cliente ${clienteId}: ${movimientos.length}`
    );
    ...
    throw new Error(
      `Error al cargar movimientos del cliente: ${error instanceof Error ? error.message : 'Error desconocido'}`
    );
  }
};

// 🔧 NUEVA FUNCIÓN: Cargar movimientos por vendedor (para optimización)
export const getMovimientosPorVendedor = async (
  vendedorId: number,
  limit?: number
): Promise<Movimiento[]> => {
  try {
    ...
    const movimientos = (data || []) as Movimiento[];
    logger.log(
      `✅ Movimientos del vendedor ${vendedorId}: ${movimientos.length}`
    );
    ...
    throw new Error(
      `Error al cargar movimientos del vendedor: ${error instanceof Error ? error.message : 'Error desconocido'}`
    );
  }
};
