...
import { Download, Filter, Edit, Trash2, ArrowLeft, Plus, Eye, Lock, AlertCircle } from 'lucide-react'

// Importaciones con rutas relativas
import { Cliente, Movimiento, calcularSaldoCliente, formatearMoneda, formatearFecha, supabase } from '../lib/supabase'
...
const EstadoCuentaView: React.FC<EstadoCuentaViewProps> = ({ 
  ...
  const [showFormularioMovimiento, setShowFormularioMovimiento] = useState(false)
  ...
  if (!cliente) {
    return (
      <div className="text-center py-8">
        ...
        <button onClick={onVolver} className="mt-4 bg-primary text-white px-4 py-2 rounded-md">
          Volver
        </button>
      </div>
    )
  ...
  const movimientosFiltrados = movimientos.filter(mov => {
    ...
    if (filtroFechaDesde && filtroFechaHasta) {
      return fechaMovimiento >= new Date(filtroFechaDesde + 'T00:00:00') && fechaMovimiento <= new Date(filtroFechaHasta + 'T23:59:59')
    }
    if (filtroFechaDesde) return fechaMovimiento >= new Date(filtroFechaDesde + 'T00:00:00')
    if (filtroFechaHasta) return fechaMovimiento <= new Date(filtroFechaHasta + 'T23:59:59')
    return true
  })
  ...
  const obtenerTituloReporte = () => {
    if (filtroFechaDesde && filtroFechaHasta) return `Estado de Cuenta del ${formatearFecha(filtroFechaDesde)} al ${formatearFecha(filtroFechaHasta)}`
    if (filtroFechaDesde) return `Estado de Cuenta desde ${formatearFecha(filtroFechaDesde)}`
    if (filtroFechaHasta) return `Estado de Cuenta hasta ${formatearFecha(filtroFechaHasta)}`
  ...
  const handleExportarPDF = () => {
    ...
    try {
      ...
      doc.text(`Tel: ${EMPRESA_INFO.telefono} | Email: ${EMPRESA_INFO.email}`, 14, 35)
      ...
      autoTable(doc, {
        startY: 100,
        head: [['Fecha', 'Tipo', 'Documento', 'Importe', 'Saldo', 'Comentario']],
        body: tableData,
        headStyles: { 
        ...
        didParseCell: function(data) {
          ...
          if (data.column.index === 3) {
            const importe = parseFloat(data.cell.text[0].replace(/[^0-9,-]/g, '').replace(',', '.'))
            if (importe < 0) {
          ...
          if (data.column.index === 4) {
            const saldo = parseFloat(data.cell.text[0].replace(/[^0-9,-]/g, '').replace(',', '.'))
            if (saldo > 0) {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.setFontSize(8)
        doc.setFont('helvetica', 'normal')
        doc.text(`Generado el ${formatearFecha(new Date().toISOString().split('T')[0])} por ${currentUser.nombre} - Página ${i} de ${pageCount}`, 14, (doc as any).internal.pageSize.height - 10)
      }
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
          ...
          <div className="text-right">
            ...
            <div className={`text-2xl font-bold ${
              saldoFinal > 0.01 ? 'text-red-600' : 
              saldoFinal < -0.01 ? 'text-green-600' : 
              'text-gray-900'
            }`}>
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="text-lg font-medium text-primary">{cliente.razon_social}</h2>
              <p className="text-sm text-gray-600">RUT: {cliente.rut} | Vendedor: {cliente.vendedor_nombre}</p>
            </div>
            ...
            {!esAdmin && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex items-center text-blue-800">
                  ...
                  <div>
                    <div className="text-sm font-medium">Vista de Solo Lectura</div>
                    <div className="text-xs text-blue-600">Puedes consultar y exportar</div>
                  </div>
                </div>
              </div>
            )}
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {movimientosConSaldo.length === 0 ? (
                <tr>
                  <td colSpan={esAdmin ? 7 : 6} className="px-6 py-8 text-center text-gray-500">
                ...
                movimientosConSaldo.map((mov) => (
                  <tr key={mov.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{formatearFecha(mov.fecha)}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        mov.tipo_movimiento === 'Venta' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'
                      }`}>
                        {mov.tipo_movimiento}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">{mov.documento || '-'}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <span className={`${(mov.saldo_acumulado || 0) > 0.01 ? 'text-red-600' : 'text-green-600'}`}>
                        {formatearMoneda(mov.saldo_acumulado || 0)}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{mov.comentario || '-'}</td>
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                Puedes consultar el estado de cuenta completo de tus clientes y exportar reportes. 
                Para crear o modificar movimientos, contacta al administrador.
              </p>
            </div>
          </div>
        </div>
